import re

def register():

    while True:
        print("\n-----------------------------------<회원가입>-----------------------------------")
        print("1 -- 학생 사용자")
        print("2 -- 교수 사용자")
        print("--------------------------------------------------------------------------------")
        userType = input("사용자 유형을 입력하시오 > ")

        # 학생 회원가입
        if userType == "1":
            # ID, PW 입력
            id, pw = getIdandPW()
            # 학과 입력
            major = getMajor()
            # 이름 입력 
            name = getName()
            # 화원가입 입력내용 확인 화면
            isConfirm = registerReview(id, pw, major, name)
            
            if isConfirm == True:
                # 회원가입 완료
                makeNewUser(id, pw, major, name, True)
            return
        
        # 교수 회원가입  
        elif userType == "2":
            # ID, PW 입력
            id, pw = getIdandPW()
            # 학과 입력
            major = getMajor()
            # 이름 입력 
            name = getName()
            # 화원가입 입력내용 확인 화면
            isConfirm = registerReview(id, pw, major, name)
            
            if isConfirm == True:
                # 회원가입 완료
                makeNewUser(id, pw, major, name, False)
            return
        
        else:
            print("올바르지 않는 형식입니다. 다시 입력해주세요.")
            continue                   
                        
                        
                        
def getIdandPW():

    idPattern = re.compile(r'^[a-zA-Z0-9]+$')
    pwPattern = re.compile(r'^[a-zA-Z0-9!@#$%^&*]+$')
    isExistingDigit = re.compile(r"\d+")

    while True:
                isExistingId = False    
                print("\n--------------------------------------------------------------------------------")
                print("사용할 ID 입력 > ", end = "")
                id = input("")

                # id 유효성 평가
                # 길이 검사
                if len(id) < 6 or len(id) > 12 :
                    print("ID는 6~12글자로 구성되어야 합니다. 다시 입력해주세요.")
                    continue

                # 숫자 알파벳 이외의 문자가 있는지 검사
                elif (not idPattern.match(id)):
                    print("알파벳과 숫자 외의 문자는 입력 불가능합니다. 다시 입력해주세요.")
                    continue

                # 숫자로만, 혹은 알파벳으로만 구성되어 있는지 검사
                elif id.isalpha():
                    print("ID는 알파벳만으로 구성될 수 없습니다. 다시 입력해주세요.")
                    continue

                elif id.isnumeric():
                    print("ID는 숫자만으로 구설될 수 없습니다. 다시 입력해주세요.")
                    continue

                else:
                    # id 중복 검사
                    userFile = open("users.txt", 'r', encoding="UTF-8")
                    userFileLines = userFile.readlines()
                    userFile.close()

                    for line in userFileLines:
                        # 사용자 정보 첫 줄인 경우
                        if len(line.strip().split("    ")) == 4:
                            # ID 이미 등록된 경우 오류 메시지 
                            if line.strip().split("    ")[0].upper() == id.upper():
                                print("이미 등록된 ID입니다. 다시 입력해주세요.")
                                isExistingId = True
                                continue

                # 중복되지 않았을 경우 
                if isExistingId == False:

                    # id 입력 완료
                    # 비밀번호 입력 시작
                    while True:
                        print("사용할 PW 입력 > ", end = "")
                        pw = input("")
                        originPw = pw

                        # pw 유효성 평가
                        # 길이 검사
                        if len(pw) < 8 or len(pw) > 15:
                            print("PW는 8~15글자로 구성되어야 합니다. 다시 입력해주세요.")
                            print("\n--------------------------------------------------------------------------------")
                            continue

                        # 허용되는 문자 외의 문자가 포함되어 있는지 검사
                        if (not pwPattern.match(pw)):
                            print("알파벳과 숫자 그리고 !,@,#,$,%,^,&,* 외의 문자는 입력 불가능합니다. 다시 입력해주세요.")
                            print("\n--------------------------------------------------------------------------------")
                            continue

                        # 숫자로만, 혹은 알파벳으로만 구성되어 있는지 검사
                        if pw.isalpha():
                            print("PW는 알파벳으로만 구성될 수 없습니다. 다시 입력해주세요.")
                            print("\n--------------------------------------------------------------------------------")
                            continue

                        elif pw.isnumeric():
                            print("PW는 숫자로만 구성될 수 없습니다. 다시 입력해주세요.")
                            print("\n--------------------------------------------------------------------------------")
                            continue
                        # 숫자가 포함되지 않은 경우
                        if not re.search(isExistingDigit, pw):
                            print("PW는 숫자를 포함해야 합니다. 다시 입력해주세요.")
                            print("\n--------------------------------------------------------------------------------")
                            continue

                        # 제한된 특수문자 포함하지 않는 경우 오류문구 출력
                        if pw.isalnum() == True:
                            print("PW는 특수문자를 포함해야 합니다. 다시 입력해주세요.")
                            print("\n--------------------------------------------------------------------------------")
                            continue
                        
                        # 비밀번호 재확인
                        pwDoubleCheck = input("사용할 PW 확인 > ")
                        
                        if originPw == pwDoubleCheck: 
                            return id, originPw
                        else:
                            print("일치하지 않습니다. 다시 입력해주세요.")
                            print("\n--------------------------------------------------------------------------------")
                            continue
                            
def getName():

    while True:
        print("이름 입력 > ", end = "")
        name = input("")
        name = name.replace(" ", "")

        if len(name) < 2 or len(name) > 5:
            print("이름은 2~5글자로 구성되어야 합니다. 다시 입력해주세요.")
            print("\n--------------------------------------------------------------------------------")
            continue

        if isKorean(name) == False:
            print("올바른 한글 이름 양식이 아닙니다. 다시 입력해주세요.")
            print("\n--------------------------------------------------------------------------------")
            continue

        return name

majors = ['컴퓨터공학부', '국어국문학과', '철학과', '기술경영학과', '물리학과']
majors2 = ['컴', '국', '철', '기', '물']

def getMajor():

    while True:
        print("학과 입력 > ", end = "")
        major = input("")
        major = major.replace(" ", "")

        # 학과 입력 유효성 검사
        if(major in majors) or (major in majors2):

            # 학과 입력이 유효한 경우 => 이름 입력
            return major
        
        else:
            print("컴퓨터공학부, 국어국문학과, 철학과, 기술경영학과, 물리학과 중에서 다시 입력해주세요.")
            print("\n--------------------------------------------------------------------------------")
            continue
        
        
def makeNewUser(id:str, pw:str, major:str, name:str, isStudent:bool):

    # 학생이면 학번이 5자리 수, 교수면 교수번호가 4자리 수
    if isStudent:
        numLen2 = 10000
        userFile = open("student_number.txt", 'r', encoding="UTF-8")
        userFileLines = userFile.readlines()
        userFile.close()

    else:
        numLen2 = 1000
        userFile = open("professor_number.txt", 'r', encoding="UTF-8")
        userFileLines = userFile.readlines()
        userFile.close()

    # 학과 번호 설정
    majorNum = 1

    if len(major) == 1:
        majorNum = majors2.index(major) + 1
    else:
        majorNum = major.index(major) + 1
        
    # 학번 설정
    studentOrProfessorNum = 0

    for line in userFileLines:
        # 학과번호가 같은 학생 사용자가 있을 때마다 studentOrProfessorNum 1씩 늘림
        if line.strip()[0] == str(majorNum):
            studentOrProfessorNum = studentOrProfessorNum + 1

    studentOrProfessorNum = majorNum * numLen2 + studentOrProfessorNum + 1    

    # 유저 파일에 기록
    userFile = open("users.txt", 'a', encoding="UTF-8")
    userFile.write(id + "    " + pw + "    " + str(studentOrProfessorNum) + "    " + name + "\n\n")
    userFile.close()

    # 학번/교수번호 파일에 기록
    if isStudent:
        userFile = open("student_number.txt", 'a', encoding="UTF-8")
        userFile.write(str(studentOrProfessorNum)+"\n")
        userFile.close()
    else:
        userFile = open("professor_number.txt", 'a', encoding="UTF-8")
        userFile.write(str(studentOrProfessorNum)+"\n")
        userFile.close()

    print("\n회원가입이 완료되었습니다. 시작화면으로 돌아갑니다.")
    print("================================================================================\n")
    return

def registerReview(id, pw, major, name):
    print("-------------------------------<입력한 내용 확인>-------------------------------")
    print("ID: " + id)
    print("PW: " + pw)
    print("학과: " + major)
    print("이름: " + name)
    print("생성 'Y' 입력 / 취소 'Y' 제외 키 입력")
    isConfirm = input("> ")
    isY = isConfirm == "Y"
    if isY == False:
        print("회원가입이 취소되었습니다. 시작화면으로 돌아갑니다.")
    return isY
    

def isKorean(text):

    # 한글 정규 표현식
    hangul = re.compile('[^ 가-힣]+')
    result = hangul.sub('', text)

    return len(result) == len(text)